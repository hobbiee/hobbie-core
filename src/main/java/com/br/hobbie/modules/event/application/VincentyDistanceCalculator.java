...
public class VincentyDistanceCalculator implements DistanceCalculator {
    ...
    @Override
    public boolean isWithinRadius(Player player, Event event) {
        var distance = getDistance(player.getMatchLatitude(), player.getMatchLongitude(), event.getLatitude(), event.getLongitude());

        return player.distanceIsWithinRadius(distance);
    }

    @Override
    public Float getDistance(float playerLatitude, float playerLongitude, float eventLatitude, float eventLongitude) {
        double U1 = Math.atan((1 - FLATTENING) * Math.tan(Math.toRadians(playerLatitude)));
        double U2 = Math.atan((1 - FLATTENING) * Math.tan(Math.toRadians(eventLatitude)));
        ...
        double longitudeDifference = Math.toRadians(eventLongitude - playerLongitude);
        ...
        do {
            ...
            longitudeDifference = Math.toRadians(eventLongitude - playerLongitude) + (1 - C) * FLATTENING * sinAlpha *
